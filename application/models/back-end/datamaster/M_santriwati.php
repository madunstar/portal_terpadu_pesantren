   ...
   function cekdataberkas($id_berkas)
    ...
    function ambilprovinsi(){
      ...
      function lihattingkatanpondokan($nis_lokal){
        return $this->db->query("SELECT `tb_pondokan_santriwati`.nis_lokal,`tb_presensi_pondokan_p`.`id_kelas_belajar`, `tb_presensi_pondokan_p`.`nama_kelas_belajar`, `tb_presensi_pondokan_p`.`pondokan`, `tb_presensi_pondokan_p`.`tingkat`, `tb_tahun_ajaran`.`tahun_ajaran` FROM `tb_pondokan_santriwati` inner join `tb_presensi_pondokan_p` on `tb_presensi_pondokan_p`.`id_kelas_belajar`=`tb_pondokan_santriwati`.`id_kelas_belajar` inner join `tb_tahun_ajaran` on `tb_tahun_ajaran`.`id_tahun`=`tb_presensi_pondokan_p`.`id_tahun` where `nis_lokal` = '$nis_lokal' order by `tb_presensi_pondokan_p`.`tingkat` asc ");
      }
