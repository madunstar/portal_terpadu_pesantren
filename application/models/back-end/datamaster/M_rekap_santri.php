...
class m_rekap_santri extends CI_Model
{
    ...
    function datapelajaran()
    {
        ...
        $this->db->join('tb_tahun_ajaran', 'tb_presensi_kelas.id_tahun = tb_tahun_ajaran.id_tahun');
        return $this->db->get();
    }
    ...
    {
      $this->db->select('*');
      $this->db->from('tb_presensi_rekap_santri_afilasi');
      $this->db->join('tb_santri', 'tb_presensi_rekap_santri_afilasi.id_santri = tb_santri.nis_lokal');
      $this->db->join('tb_presensi_kelas', 'tb_presensi_rekap_santri_afilasi.id_kelas = tb_presensi_kelas.id_kelas_belajar');
      $this->db->join('tb_pelajaran', 'tb_presensi_rekap_santri_afilasi.id_pelajaran = tb_pelajaran.id_pelajaran');
      $this->db->join('tb_mata_pelajaran', 'tb_pelajaran.id_mata_pelajaran = tb_mata_pelajaran.id_mata_pelajaran');
      $this->db->where('tb_presensi_rekap_santri_afilasi.id_pelajaran', $pel);
      $this->db->where('tb_presensi_rekap_santri_afilasi.id_kelas', $kel);
      $this->db->where('tb_presensi_rekap_santri_afilasi.tanggal_rekap', $tgl);
      return $this->db->get();
    }

    function datasantri($kel,$tgl,$pel){

      return $this->db->query('select * from tb_kelas_santri inner join tb_santri on tb_kelas_santri.nis_lokal = tb_santri.nis_lokal where tb_kelas_santri.id_kelas_belajar ='.$kel.' and not exists (select * from tb_presensi_rekap_santri_afilasi where tb_presensi_rekap_santri_afilasi.id_santri = tb_santri.nis_lokal and id_kelas = '.$kel.' and tanggal_rekap ="'.$tgl.'" and id_pelajaran ='.$pel.')');
    }
    ...
    {
        return $this->db->insert('tb_presensi_rekap_santri_afilasi',$array);
    }
    ...
    function hapus($id_rekap)
    {
        ...
        return $this->db->delete('tb_presensi_rekap_santri_afilasi');
    }
