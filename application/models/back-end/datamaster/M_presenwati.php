   ...
   function cekdataberkas($id_berkas)
    ...
    function hapusjadwal($id_jadwal)
     ...
     public function lihatdataajax()
     {
         ...
         $sql = "SELECT * ";
         $sql.=" FROM tb_presensi_kelas_p inner join tb_guru on tb_guru.nip_guru=tb_presensi_kelas_p.nip_guru
                 inner join tb_kelas on tb_kelas.kd_kelas=tb_presensi_kelas_p.kd_kelas
                 inner join tb_tahun_ajaran on tb_tahun_ajaran.id_tahun=tb_presensi_kelas_p.id_tahun
         WHERE 1=1 ";
         ...
         foreach($query->result_array() as $row) {  // preparing an array
             ...
             $akd = " <a href='".base_url('admin/datamaster/lihatkelasbelawati?id='.$row['id_kelas_belajar'].'')."' class='btn btn-primary btn-xs' title='Lihat'><i class='fa fa-eye'></i></a>
             <a href='".base_url('admin/datamaster/editkelasbelawati?id='.$row['id_kelas_belajar'].'')."' class='btn btn-warning btn-xs' title='Ubah'><i class='fa fa-edit'></i></a>
             <a href='#' class='btn btn-danger btn-xs hapus' title='Hapus' id='".$row['id_kelas_belajar']."'><i class='fa fa-trash-o'></i></a>
             <a href='".base_url('admin/datamaster/printkelasafiwati?id='.$row['id_kelas_belajar'].'')."' class='btn btn-info btn-xs print' title='Cetak' id='".$row['id_kelas_belajar']."'><i class='fa fa-print'></i></a>";
             $nestedData[] = $akd;
             ...
             $nestedData[] = "<a href='".base_url('admin/datamaster/jadwalafiwati?id='.$row['id_kelas_belajar'].'')."' class='btn btn-primary btn-xs' title='Lihat Jadwal'><i class='fa fa-clock-o'></i> Jadwal</a>";
             ...
             $nestedData[] = "<a href='".base_url('admin/datamaster/lihatkelasbelajarsantriwati?id='.$row['id_kelas_belajar'].'')."' class='btn btn-primary btn-xs' title='Lihat Daftar Santriwati'><i class='fa fa-list'></i> Santriwati</a>";
             $data[] = $nestedData;
             $no++;
         }
         $json_data = array(
