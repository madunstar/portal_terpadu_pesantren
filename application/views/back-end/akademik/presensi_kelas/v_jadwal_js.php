<script>
   ...
   $(".hapus").click(function (e) {
    ...
    $.confirm({
        ...
        buttons: {
            hapus: {
                ...
                btnClass: 'btn-green',
                action: function(){
                    window.location.assign("<?php echo base_url() ?>admin/santriakd/hapusjadwalafilasi?id="+v_id+"&idkelas="+v_idkelasbelajar);
                }
            },
            batal: function () {
    ...
    $("#tambahjadwal").click(function(e) {
        var v_url = "<?php echo base_url() ?>admin/santriakd/tambahjadwalafilasi";
        var v_idkelasbelajar = "<?php echo $jadwal['id_kelas_belajar'] ?>";
        $.ajax({
				type: 'POST',
				url: v_url,
                    ...
                    $('#tambahjadwalproses').click(function (e) {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                $("#myModal").hide();
                                window.location.assign("<?php echo base_url() ?>admin/santriakd/jadwalafilasi?id=<?php echo $jadwal['id_kelas_belajar'] ?>&msg=1")
                                //isidata();
    ...
    $(".editjadwal").click(function(e) {
        var v_url = "<?php echo base_url() ?>admin/santriakd/editjadwalafilasi";
        var v_idkelasbelajar = "<?php echo $jadwal['id_kelas_belajar'] ?>";
        var v_id = this.id;
        $.ajax({
				type: 'POST',
                ...
                complete: function () {
                    ...
                    $('#editjadwalproses').click(function (e) {
                        var v_url = "<?php echo base_url() ?>admin/santriakd/editjadwalafilasiproses";
                        var v_idkelasbelajar = "<?php echo $jadwal['id_kelas_belajar'] ?>";
                        var v_mata_pelajaran = $('#mata_pelajaran').val();
                        var v_jam = $('#jam').val();
                        var v_guru = $('#guru').val();
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                $('body').css('padding-right', '');
                                $("#myModal").hide();
                                window.location.assign("<?php echo base_url() ?>admin/santriakd/jadwalafilasi?id=<?php echo $jadwal['id_kelas_belajar'] ?>&ed=1")
                                //isidata();
                                //pesanpop('Pesan !', 'Berhasil menambah data Gudang.', 'success')
                            }
                        });
